...
VENDORED (2026-07-19, upstream main) with two local patches for the
ThemeParkWaits streaming fetch path — re-apply if re-vendoring:
  1. ``_IterToStream.read()`` skips EMPTY chunks (a native ``iter_content``
     may yield ``b""``; upstream indexed it -> IndexError).
  2. Per-byte set-literal membership tests replaced with int-constant
     comparisons (review 2026-07-19: avoid any chance of per-character set
     allocation on the parser's hottest path under a non-compacting GC).
"""
...
class _IterToStream:
    ...
    def read(self):
        ...
        # LOCAL PATCH 1: while-loop (not if) so EMPTY chunks are skipped —
        # a native iter_content may legally yield b"" (socket boundary), and
        # indexing it raised IndexError, killing the whole fetch attempt.
    ...
    def fast_forward(self, closer, *, return_object=False):
        ...
        while close_stack:
            ...
            if ignore_next:
                ...
                pass
            elif char == 125 or char == 93:   # LOCAL PATCH 2: '}' / ']'
    ...
    def next_value(self, endswith=None):
        ...
        while True:
            ...
            if not in_string:
                ...
                if char == endswith or char == 93 or char == 125:  # LOCAL PATCH 2: ']' / '}'
                    self.last_char = char
                    if len(buf) == 0:
                        return None
