...
- **Hardware checklist T035–T043 + T045** — device-only. USER runs on a Matrix Portal S3. **First hardware run 2026-06-22 (`scripts/deploy.sh` → CP 9.1.0 board):** the app **boots and runs live** — WiFi connect, clock (HTTP-Date google fallback), mDNS `themeparkwaits.local`, 141-park list, Magic Kingdom ride data, display + data loops, **1.8 MB free heap** (T037 core + T043 memory effectively pass). **Bugs found (device-only; simulator can't catch):**
  - **(A) Web config UI — RESOLVED 2026-06-22; T038 PASSES.** The config page now serves on hardware over **both raw IP and `themeparkwaits.local`** (HTTP 200, real UI). Took FOUR device-only layers, each catchable only on the board: (A1) `@route` set `func._route_info` — illegal on CircuitPython [LIB fix]; (A2) the fix then read `func.__name__` — also unavailable [LIB fix, marker-object pattern]; (A3) the CircuitPython adapter used obsolete adafruit_httpserver v1.x (`HTTPServer`) while the board ships **4.4.4** (`Server`) [LIB port to 4.x]; (A4) the adapter needs a `socketpool` from `wifi.radio` that the app must inject [APP fix in `create_web_server`, commit b6a7dd3]. Library fixes on branch `fix/route-circuitpython` (commits 484e368, ba89e16, 5101dcc), verified on-device. See `SCROLLKIT_NOTES.md` #4.
