...
**Remaining:**
- **Hardware checklist T035–T043 + T045** — device-only. USER runs on a Matrix Portal S3. **First hardware run 2026-06-22 (`scripts/deploy.sh` → CP 9.1.0 board):** the app **boots and runs live** — WiFi connect, clock (HTTP-Date google fallback), mDNS `themeparkwaits.local`, 141-park list, Magic Kingdom ride data, display + data loops, **1.8 MB free heap** (T037 core + T043 memory effectively pass). **Bugs found (device-only; simulator can't catch):**
  - **(A) Web config UI — RESOLVED 2026-06-22; T038 PASSES.** The **dynamic** config page now serves on hardware (HTTP 200, 29.5 KB: park selectors + settings form + Save) over **both raw IP and `themeparkwaits.local`**. Took FIVE device-only layers, each catchable only on the board: (A1) `@route` set `func._route_info` — illegal on CircuitPython [LIB]; (A2) the fix then read `func.__name__` — also unavailable [LIB, marker-object pattern]; (A3) the CircuitPython adapter used obsolete adafruit_httpserver v1.x (`HTTPServer`) while the board ships **4.4.4** (`Server`) [LIB port to 4.x]; (A4) **CircuitPython never calls `__init_subclass__`**, so the route registry stayed empty → NO dynamic routes registered → server served only static files → `/` fell through to a stale static `index.html` stub showing "Park Status: Loading…" and `/settings`→404. Fixed: `_iter_routes` discovers routes from live `@route` markers [LIB, commit 09f7e23]; (A5) the adapter needs a `socketpool` from `wifi.radio` the app must inject [APP, commit b6a7dd3]. Library fixes on branch `fix/route-circuitpython` (484e368, ba89e16, 5101dcc, 09f7e23), all verified on-device. See `SCROLLKIT_NOTES.md` #4. **Cleanup:** the stale `src/www/index.html` stub (the "Park Status: Loading…" page) was removed 2026-06-22; `/` serves only the dynamic page now.
    - **(A′) Web config UI REWRITTEN to native `adafruit_httpserver` — RESOLVED 2026-06-23 (commit d4a1501), device-verified.** Decision (multi-model review + empirical): the library web abstraction (`SLDKWebServer`/`ServerAdapter`/`MockRequest`/`MockResponse`/`@route` registry) caused all SIX CircuitPython-only traps above AND masked the v1→v4 API drift; since `adafruit_httpserver` is itself cross-platform (runs on desktop CPython with the stdlib `socket` module as its pool), the abstraction was redundant. `src/web/config_server.py` now talks to `adafruit_httpserver.Server` directly — the SAME server object on desktop + device, only the socket pool differs (`socketpool.SocketPool(wifi.radio)` on CP / stdlib `socket` on desktop). Routes `/`, `/settings`[GET,POST], `/update`[POST]; static from `src/www` via `root_path`. **POST `/settings` → in-memory apply + content rebuild (no fetch, FR-004) → clean HTTP 303 → `/`** (NOT the old heavy styled meta-refresh page — the likely on-device POST freeze cause). Reuses the old `_apply`/form-HTML verbatim (now module funcs `apply_settings`/`render_page`). **Device-verified on `192.168.68.121` (CP 9.1.0):** GET `/` HTTP 200 / 29.5 KB (1.5s); GET `/style.css` 200 (0.09s); GET via `themeparkwaits.local` 200 (~6.4s mDNS); **POST `/settings` HTTP 303 → `/` in 0.17s — no freeze**; re-GET shows the POSTed values selected (round-trip applies). Free heap 1.77 MB. (Disk persistence not re-checked here — CIRCUITPY was host-mounted/read-only during the test, finding C; in-memory apply confirmed.) **The library web layer is no longer on the app's config path → branch `fix/route-circuitpython` (A1–A4 fixes) is now UNUSED by this app.** 30 desktop tests pass (web tests rewritten to drive the real native server over real HTTP on the stdlib pool).
  - **(B) OTA disabled — `os.path` AttributeError** (APP, ota_glue.py): CircuitPython `os` has no `path`. **FIXED 2026-06-22** (string ops + `os.stat`); redeploy to verify on-device.
  - **(C) Read-only filesystem while USB-mounted** (CONFIG): device can't persist settings/OTA — eject the CIRCUITPY drive (or a `boot.py` `storage.remount`) for T039/OTA tests.
  - **(D) Slow boot (~30-60s): EINPROGRESS on the first HTTPS socket** (LIBRARY robustness): cloudflare fails 3x before google works; not fatal. See `SCROLLKIT_NOTES.md` #5.
  - **(E) `<domain>.local` resolved once then died** (APP, app.py): `_init_network()` called `advertise()` but discarded the returned `mdns.Server`, which was then garbage-collected → the responder stopped answering. **FIXED 2026-06-22** (`self.mdns_server = advertise(...)`, commit 29eef52); verified `.local` now resolves 4/4 on-device. (Resolution is ~6s/query — slow but consistent.)
  - T035 (AP onboarding) still blocked on the unwritten onboarding code; T041/T042 (OTA) blocked on the `live` branch + (B)/(C).
...
- [X] **T038** Config page reachable at `http://<domain_name>.local/` **and** by raw IP — **VERIFIED on-device 2026-06-22**: `curl http://192.168.68.118/` and `curl http://themeparkwaits.local/` both return HTTP 200 (real config UI: Park Status / Device Settings / WiFi Setup). mDNS helper + `SLDKWebServer` coexist. Required the 4-layer web fix (finding A above) + the socket-pool injection.
