...
- **App framework**: subclass `ScrollKitApp(enable_web=True, update_interval=600)`; `setup()` = boot sequence (splash→wifi→OTA-install→clock→fetch), `update_data()` = 10-min refresh (drops to a 60 s retry while data is stale, back to 10 min on a fully healthy fetch), content via `self.content_queue`.
